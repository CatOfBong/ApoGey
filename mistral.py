...
def rocket_motion(t, state, m0, m_final, thrust, burn_time):
    ...
    rho, c = atmosphere(max(y, 0), ROCKET_PARAMS['g0'])
...
apogee_event.terminal = True  # Остановить интегрирование при достижении
apogee_event.direction = -1   # Только при переходе от положительной к отрицательной
...
for i, t in enumerate(time_points):
    ...
    rho, c = atmosphere(max(altitudes[i], 0), ROCKET_PARAMS['g0'])
...
gs = mpl.gridspec.GridSpec(4, 2, height_ratios=[1, 1, 1, 0.7])
axs = [
    plt.subplot(gs[0, 0]),
    plt.subplot(gs[0, 1]),
    plt.subplot(gs[1, 0]),
...
ax.set_ylabel('Коэффициент сопротивления (Cd)')
ax.set_title('Зависимость коэффициента сопротивления от числа Маха')
ax.grid(True, alpha=0.3)
ax.axvline(x=1.0, color='b', linestyle='--', alpha=0.7)
ax.text(0.5, 0.85, 'Трансзвуковая зона', transform=ax.transAxes, fontsize=12,
        bbox=dict(facecolor='white', alpha=0.8))
ax.annotate('Максимум сопротивления\nпри M=1.0', xy=(1.0, 1.1),
            xytext=(1.5, 0.9), arrowprops=dict(arrowstyle="->"),
